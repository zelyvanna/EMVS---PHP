...
<?php		
	...
	if(is_uploaded_file($_FILES['maPhoto']['tmp_name'])) {
		$path_parts = pathinfo($_FILES['maPhoto']['name']);
		if($path_parts['extension'] == 'jpeg'){
			echo "Le fichier ".$_FILES['maPhoto']['name'];
			echo " chargé avec succès";
			copy($_FILES['maPhoto']['tmp_name'], "./img/".$_POST["PK_film"].".jpeg");
		}else{
			echo $path_parts['extension']." n'est pas un fichier jpeg";
		}
	}
	getFooter();